...
template <typename Key, member_hook Key::* Hook, typename Compare, size_t NodeSize, key_search Search, with_debug Debug> class node;
template <typename Key, member_hook Key::*, typename Compare, size_t NodeSize> class validator;
...
public:
     ...
     * for it will be reserved by class node.
     */
    member_hook* keys[0];

    node_base(const node_base&) = delete;
    node_base(unsigned short n, unsigned short f) noexcept : num_keys(n), flags(f) {}
 ...
 */
class member_hook {
    template <typename K, member_hook K::* H, typename C, size_t NS> friend class validator;
    template <typename K, member_hook K::* H, typename C, size_t NS, key_search KS, with_debug D> friend class node;
...
template <typename Key, member_hook Key::* Hook, typename Compare, size_t NodeSize, key_search Search, with_debug Debug = with_debug::no>
...
public:
    friend class node<Key, Hook, Compare, NodeSize, Search, Debug>;
    friend class validator<Key, Hook, Compare, NodeSize>;

    using node = class node<Key, Hook, Compare, NodeSize, Search, Debug>;
...
private:
    ...
    struct cursor {
        ...
        void descend() noexcept { n = n->_kids[idx]; }
    ...
    void maybe_init_empty_tree() {
        if (_root == nullptr) {
            ...
            n->_base.flags |= node::NODE_ROOT | node::NODE_LEFTMOST | node::NODE_RIGHTMOST;
            do_set_root(*n);
            do_set_left(*n);
            do_set_right(*n);
        }
...
public:
    ...
    const_iterator find(const K& k, Compare cmp) const {
        ...
        if (_root == nullptr || !key_lower_bound(k, cmp, cur)) {
            return cend();
        }
    ...
    void clone_from(const tree& t, Cloner&& cloner, Deleter&& deleter) {
        ...
        if (t._root != nullptr) {
            ...
            left->_base.flags |= node::NODE_LEFTMOST;
            do_set_left(*left);
            right->_base.flags |= node::NODE_RIGHTMOST;
            do_set_right(*right);
            _root->_base.flags |= node::NODE_ROOT;
            do_set_root(*_root);
        }
    }
    ...
    protected:
        ...
        static constexpr key_index npos = NodeSize;
        ...
        node_ptr revalidate() noexcept {
            assert(!is_end());
            node_ptr n = node::from_hook(_hook);
             ...
             * at us, it is valid.
             */
            if (_idx >= n->_base.num_keys || n->_base.keys[_idx] != _hook) {
        ...
        operator cursor() noexcept {
            ...
            if (is_end()) {
                assert(_tree->_root != nullptr);
                ret.n = _tree->_right;
                ret.idx = ret.n->_base.num_keys;
            } else {
                ret.n = const_cast<node*>(revalidate());
                ret.idx = _idx;
            }
    ...
    public:
        ...
        iterator_base& operator++() noexcept {
            node_ptr n = revalidate();

            if (n->is_leaf()) {
                if (_idx < n->_base.num_keys - 1u) {
                    _idx++;
                } else if (n->is_rightmost() || n->is_root()) {
                    _idx = npos;
                    _tree = n->_leaf_tree;
                    return *this;
                } else {
        ...
        iterator_base& operator--() noexcept {
            if (is_end()) {
                node_ptr n = _tree->_right;
                assert(n->_base.num_keys > 0);
                _idx = n->_base.num_keys - 1u;
                _hook = n->_base.keys[_idx];
                return *this;
            }
    ...
    public:
        ...
        bool operator==(const iterator& o) const noexcept { return super::operator==(const_iterator(o)); }
    ...
    public:
        ...
        bool operator==(const const_iterator& o) const noexcept { return super::operator==(iterator(o)); }
        ...
        tree* tree_if_singular() noexcept {
            node* n = super::revalidate();
            return ((n->is_root() && n->is_leaf() && n->_base.num_keys == 1) ? n->_parent.t : nullptr);
        }
        ...
        iterator erase_and_dispose(Disp&& disp) noexcept {
            ...
            if (node::from_hook(cur._hook) == n && cur._idx > 0) {
                cur._idx--;
            }
            return cur;
        }
    ...
    bool empty() const noexcept { return _root == nullptr || _root->_base.num_keys == 0; }
    ...
    const_iterator cbegin() const noexcept {
        ...
        return const_iterator(*_left->_base.keys[0], 0);
    }
    ...
    size_t calculate_size() const noexcept {
        return _root == nullptr ? 0 : _root->size_slow();
    }

    size_t external_memory_usage() const noexcept {
        return _root == nullptr ? 0 : _root->external_memory_usage();
    }
 ...
 */
template <typename Key, member_hook Key::* Hook, typename Compare, size_t NodeSize, key_search Search, with_debug Debug>
class node {
    friend class tree<Key, Hook, Compare, NodeSize, Search, Debug>;
    friend class validator<Key, Hook, Compare, NodeSize>;

    using tree = class tree<Key, Hook, Compare, NodeSize, Search, Debug>;
    ...
    /*
     * The node_base has keys[] field, but of zero size, because it should be
     * NodeSize-agnostic. Thus the real memory for key's pointers is reserved
     * here.
     */
    union {
        node_base _base;
        char _[sizeof(node_base) + NodeSize * sizeof(member_hook*)];
     ...
     * // need the tree pointer to update its _left/_right on move.
     *
     ...
     *
     * See leaf_node_size/inner_node_size constants.
     */
    union {
        node* _kids[0];
        tree* _leaf_tree;
...
public:
    /*
     * Leaf node is fully described by the above. It's layout is
     *
     * _parent        (pointer)
     * _base.flags    (short)
     * _base.num_keys (short)
     * ...            (compiler's alignment gap)
     * _base.keys     (N pointers, thanks to union's _ array)
     * _leaf_tree     (pointer)
     */
    static constexpr size_t leaf_node_size = sizeof(node);
    /*
     * Inner nodes don't need the _leaf_tree pointer, but
     * do need N+1 kid pointers instead. The layout is lile
     *
     * _parent        (pointer)
     * _base.flags    (short)
     * _base.num_keys (short)
     ...
     * _kids          (N + 1 pointers, just becase allocation size)
     */
    static constexpr size_t inner_node_size = sizeof(node) - sizeof(tree*) + (NodeSize + 1) * sizeof(node*);

private:
    bool is_root() const noexcept { return _base.flags & NODE_ROOT; }
    bool is_leaf() const noexcept { return _base.flags & NODE_LEAF; }
    bool is_leftmost() const noexcept { return _base.flags & NODE_LEFTMOST; }
    bool is_rightmost() const noexcept { return _base.flags & NODE_RIGHTMOST; }
    ...
    void unlink_corner_leaf() noexcept {
        ...
        switch (_base.flags & (NODE_LEFTMOST | NODE_RIGHTMOST)) {
            case 0:
                break;
            case NODE_LEFTMOST:
                assert(p->_base.num_keys > 0 && p->_kids[0] == this);
                x = p->_kids[1];
                _base.flags &= ~NODE_LEFTMOST;
                x->_base.flags |= NODE_LEFTMOST;
                _leaf_tree->do_set_left(*x);
                break;
            case NODE_RIGHTMOST:
                assert(p->_base.num_keys > 0 && p->_kids[p->_base.num_keys] == this);
                x = p->_kids[p->_base.num_keys - 1];
                _base.flags &= ~NODE_RIGHTMOST;
                x->_base.flags |= NODE_RIGHTMOST;
    ...
    static node* from_hook(const member_hook* hook) noexcept {
        return boost::intrusive::get_parent_from_member(hook->_node, &node::_base);
    }
...
public:
    explicit node(unsigned short flags) noexcept : _base(0, flags) { }

    node(node&& other) noexcept : _base(0, other._base.flags) {
        if (is_leaf()) {
            if (is_leftmost()) {
                other._leaf_tree->do_set_left(*this);
            }

            if (is_rightmost()) {
                other._leaf_tree->do_set_right(*this);
            }

            other._base.flags &= ~(NODE_LEFTMOST | NODE_RIGHTMOST);
        } else {
...
private:
    friend size_t size_for_allocation_strategy(const node& n) noexcept {
        return n.is_leaf() ? leaf_node_size : inner_node_size;
    }

    static node* construct(size_t size, unsigned short flags) {
        void* mem = current_allocator().alloc(&get_standard_migrator<node>(), size, alignof(node));
        return new (mem) node(flags);
    }

    static node* create_leaf() { return construct(leaf_node_size, NODE_LEAF); }
    static node* create_inner() { return construct(inner_node_size, 0); }
    static node* create_node(bool leaf) { return leaf ? create_leaf() : create_inner(); }
    ...
    kid_index index_for(const member_hook* hook) const noexcept {
        return _base.index_for(hook);
    }
    ...
    // Occupu the hole at \at after key (and kid) removal
    ...
    void insert_into_root(member_hook& key, node* kid, prealloc& nodes) noexcept {
        ...
        _base.flags &= ~node::NODE_ROOT;
        nr->_base.flags |= node::NODE_ROOT;
        t->do_set_root(*nr);
    }
