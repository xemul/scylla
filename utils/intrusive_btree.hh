...
private:
    bool is_root() const noexcept { return _base.flags & node_base::NODE_ROOT; }
    bool is_leaf() const noexcept { return _base.flags & node_base::NODE_LEAF; }
    bool is_leftmost() const noexcept { return _base.flags & node_base::NODE_LEFTMOST; }
    bool is_rightmost() const noexcept { return _base.flags & node_base::NODE_RIGHTMOST; }
    bool is_linear() const noexcept { return make_linear_root && _base.flags & node_base::NODE_LINEAR; }
...
public:
    ...
    node(size_t cap, node&& other) noexcept : _base(0, cap, other._base.flags) {
        if (is_leaf()) {
            if (is_leftmost()) {
                other.leaf_tree()->do_set_left(*this);
            }

            if (is_rightmost()) {
                other.leaf_tree()->do_set_right(*this);
            }
