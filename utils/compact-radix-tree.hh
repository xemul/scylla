...
static inline unsigned find_in_array(uint8_t val, const uint8_t* arr) {
    std::abort();
}
...
private:
    ...
    /*
     * All nodes must start with this. But direct inclusion of this struct into
     * a node is not required, as it's size is not pointer-aligned, so compiler
     * will put a gap after it. If the subsequent fields are small, it makes
     * perfect sence just to re-declare the head's fields and follow them with
     * more ones thus re-using this gap.
     */
    struct node_head {
         ...
         * it will cast iself to inner/leaf node.
         */
        template <typename NT>
        NT& as() noexcept { return *reinterpret_cast<NT*>(this); }

        template <typename NT>
        const NT& as() const noexcept { return *reinterpret_cast<const NT*>(this); }
        ...
        const T* get(index_t idx, unsigned depth) const noexcept {
            return depth == leaf_depth ? as<leaf_node>().get(idx) : as<inner_node>().get(idx, depth);
        }
        ...
        lower_bound_res lower_bound(index_t& idx, unsigned depth) const noexcept {
            ...
            depth = real_depth;
            if (depth == leaf_depth) {
                const T* val = as<leaf_node>()._base.lower_bound(idx, depth).first;
                return std::pair(val, this);
            } else {
                return as<inner_node>()._base.lower_bound(idx, depth);
            }
        }
        ...
        bool visit(Visitor&& v, unsigned depth) const {
            ...
            if (v(*this, depth, true)) {
                if (depth == leaf_depth) {
                    ret = as<leaf_node>()._base.visit(v, depth);
                } else {
                    ret = as<inner_node>()._base.visit(v, depth);
                }
                v(*this, depth, false);
            }
            return ret;
        ...
        allocate_res alloc(index_t idx, unsigned depth) {
            if (depth == leaf_depth) {
                return as<leaf_node>()._base.alloc(idx, depth);
            } else {
                return as<inner_node>()._base.alloc(idx, depth);
            }
        }

        erase_result erase(index_t idx, unsigned depth, erase_mode erm) noexcept {
            if (depth == leaf_depth) {
                return as<leaf_node>()._base.erase(idx, depth, erm);
            } else {
                return as<inner_node>()._base.erase(idx, depth, erm);
            }
        }
        ...
        erase_result weed(Fn&& filter, unsigned depth) {
            if (depth == leaf_depth) {
                return as<leaf_node>()._base.weed(filter, depth);
            } else {
                return as<inner_node>()._base.weed(filter, depth);
            }
        }

        node_head* grow(unsigned depth) {
            if (depth == leaf_depth) {
                return as<leaf_node>()._base.template grow<leaf_node>();
            } else {
                return as<inner_node>()._base.template grow<inner_node>();
            }
        }

        node_head* shrink(unsigned depth) {
            if (depth == leaf_depth) {
                return as<leaf_node>()._base.template shrink<leaf_node>();
            } else {
                return as<inner_node>()._base.template shrink<inner_node>();
            }
        }

        void free(unsigned depth) noexcept {
            if (depth == leaf_depth) {
                leaf_node::free(as<leaf_node>());
            } else {
                inner_node::free(as<inner_node>());
            }
        }
        ...
        const T* lower_bound(index_t& idx) const noexcept {
            return as<leaf_node>()._base.lower_bound(idx, leaf_depth).first;
        }
        ...
        void set_lower(node_index_t idx, node_head* n) noexcept {
            as<inner_node>().set_lower(idx, n);
        }

        const node_head_ptr* pop_lower(node_index_t& idx) const noexcept {
            return as<inner_node>().pop_lower(idx);
        }
    };
    ...
    struct node_base {
        ...
        static size_t node_size(node_kind kind) noexcept {
            return kind == Tx::kind ? sizeof(Tx) : node_size<Sentinel, Ts...>(kind);
        }
        ...
        template <bool Sentinel>
        void init() noexcept { __builtin_unreachable(); }
        ...
        template <bool Sentinel>
        void init(node_base&& o) noexcept { __builtin_unreachable(); }

        template <bool Sentinel, typename Tx, typename... Ts>
        void init(node_base&& o) noexcept {
            if (_head._kind == Tx::kind) {
                as<Tx>().init(std::move(o.as<Tx>()));
            } else {
                init<Sentinel, Ts...>(std::move(o));
            }
        }

        node_base(node_base&& o) noexcept : _head(std::move(o._head)) {
            init<true, Layouts...>(std::move(o));
        }
        ...
        template <bool Sentinel>
        const Slot* find(node_index_t idx) const noexcept { __builtin_unreachable(); }

        template <bool Sentinel, typename Tx, typename... Ts>
        const Slot* find(node_index_t idx) const noexcept {
            return _head._kind == Tx::kind ? as<Tx>().find(idx) : find<Sentinel, Ts...>(idx);
        }
        ...
        template <bool Sentinel, typename Tx, typename... Ts>
        lower_bound_res lower_bound(index_t& idx, unsigned depth) const noexcept {
            return _head._kind == Tx::kind ? as<Tx>().lower_bound(idx, depth) : lower_bound<Sentinel, Ts...>(idx, depth);
        }
        ...
        lower_bound_res lower_bound(index_t& idx, unsigned depth) const noexcept {
            ...
            return lower_bound<true, Layouts...>(idx, depth);
        }

        // Erase an index from node
        template <bool Sentinel>
        erase_result erase(index_t idx, unsigned depth, erase_mode erm) noexcept { __builtin_unreachable(); }

        template <bool Sentinel, typename Tx, typename... Ts>
        erase_result erase(index_t idx, unsigned depth, erase_mode erm) noexcept {
            return _head._kind == Tx::kind ? as<Tx>().erase(idx, depth, erm) : erase<Sentinel, Ts...>(idx, depth, erm);
        }

        erase_result erase(index_t idx, unsigned depth, erase_mode erm) noexcept {
            return erase<true, Layouts...>(idx, depth, erm);
        }

        // Weed node with filter
        template <bool Sentinel, typename Fn>
        erase_result weed(Fn&& filter, unsigned depth) { __builtin_unreachable(); }

        template <bool Sentinel, typename Fn, typename Tx, typename... Ts>
        erase_result weed(Fn&& filter, unsigned depth) {
            return _head._kind == Tx::kind ? as<Tx>().weed(filter, depth) : weed<Sentinel, Fn, Ts...>(filter, depth);
        }

        template <typename Fn>
        erase_result weed(Fn&& filter, unsigned depth) {
            return weed<true, Fn, Layouts...>(filter, depth);
        }
        ...
        template <bool Sentinel>
        allocate_res alloc(index_t idx, unsigned depth) { __builtin_unreachable(); }

        template <bool Sentinel, typename Tx, typename... Ts>
        allocate_res alloc(index_t idx, unsigned depth) {
            return _head._kind == Tx::kind ? as<Tx>().alloc(idx, depth) : alloc<Sentinel, Ts...>(idx, depth);
        }

        allocate_res alloc(index_t idx, unsigned depth) {
            return alloc<true, Layouts...>(idx, depth);
        }
        ...
        template <bool Sentinel>
        Slot* reserve(node_index_t idx) noexcept { __builtin_unreachable(); }

        template <bool Sentinel, typename Tx, typename... Ts>
        Slot* reserve(node_index_t idx) noexcept {
            return _head._kind == Tx::kind ? as<Tx>().reserve(idx) : reserve<Sentinel, Ts...>(idx);
        }

        Slot* reserve(node_index_t idx) noexcept {
            return reserve<true, Layouts...>(idx);
        }
        ...
        template <bool Sentinel>
        const Slot* pop(node_index_t& idx) const noexcept { __builtin_unreachable(); }

        template <bool Sentinel, typename Tx, typename... Ts>
        const Slot* pop(node_index_t& idx) const noexcept {
            return _head._kind == Tx::kind ? as<Tx>().pop(idx) : pop<Sentinel, Ts...>(idx);
        }

        const Slot* pop(node_index_t& idx) const noexcept {
            return pop<true, Layouts...>(idx);
        }

        // Visiting
        template <bool Sentinel, typename Visitor>
        bool visit(Visitor&& v, unsigned depth) const { __builtin_unreachable(); }

        template <bool Sentinel, typename Visitor, typename Tx, typename... Ts>
        bool visit(Visitor&& v, unsigned depth) const {
            return _head._kind == Tx::kind ? as<Tx>().visit(v, depth) : visit<Sentinel, Visitor, Ts...>(v, depth);
        }

        template <typename Visitor>
        bool visit(Visitor&& v, unsigned depth) const {
            return visit<true, Visitor, Layouts...>(v, depth);
        }
        ...
        node_head* grow() {
            ...
            return grow<Sentinel, NT, Tn, Ts...>();
        }

        template <typename NT>
        node_head* grow() {
            return grow<true, NT, Layouts...>();
        }
        ...
        node_head* shrink() {
            if (_head._kind == Tx::kind) {
                ...
                as<Tx>().shrink(nn->_base);
                return &nn->_base._head;
            }

            return shrink<Sentinel, NT, Tx, Ts...>();
        }

        template <typename NT>
        node_head* shrink() {
            return shrink<true, NT, Layouts...>();
        }
    };
    ...
    struct table_layout_base {
        ...
        void init() noexcept { _data.init(); }

        index_t tree_index(unsigned pos) const noexcept {
            return tree::tree_index(_data._prefix, _data._idx[pos]);
        }

        void init(table_layout_base&& o) noexcept {
            _data.init(std::move(o._data));
        }
        ...
        template <typename Fn>
        void weed(Fn&& filter, unsigned depth) {
            for (unsigned i = 0; i < Table::size; i++) {
                if (_data.has(i)) {
                    if (weed_from_slot(tree_index(i), &_data._slots[i], filter, depth + 1)) {
                        _data.unset(i);
                    }
                }
            }
        ...
        lower_bound_res lower_bound(index_t& idx, unsigned depth) const noexcept {
            ...
            for (unsigned i = 0; i < Table::size; i++) {
                if (_data.has(i)) {
                    if (_data._idx[i] == ni) {
                        lower_bound_res ret = lower_bound_at(tree_index(i), &_data._slots[i], depth, idx);
             ...
             */
            idx = 0;
            return lower_bound_at(tree_index(ui), &_data._slots[ui], depth, idx);
        }
        ...
        bool visit(Visitor&& v, unsigned depth) const {
            ...
            for (unsigned i = 0; i < sz; i++) {
                ...
                if (!visit_slot(v, tree_index(pos), &_data._slots[pos], depth)) {
                    return false;
                }
            }
            return true;
    ...
    struct map_layout {
        ...
        index_t tree_index(node_index_t i) const noexcept {
            return tree::tree_index(_data._prefix, i);
        }
        ...
        void init(map_layout&& o) noexcept {
            for (unsigned i = 0; i < Map::index_size; i++) {
        ...
        erase_result weed(Fn&& filter, unsigned depth) {
            for (unsigned i = 0; i < Map::index_size; i++) {
                uint8_t mi = _data._map[i];
                if (mi != unused) {
                    if (weed_from_slot(tree_index(i), &_data._slots[mi], filter, depth + 1)) {
                        free_slot(i, mi);
                    }
                }
            }
        ...
        lower_bound_res lower_bound(index_t& idx, unsigned depth) const noexcept {
            ...
            uint8_t mi = _data._map[ni];
            if (mi != unused) {
                lower_bound_res ret = lower_bound_at(tree_index(ni), &_data._slots[mi], depth, idx);
                if (ret.first != nullptr) {
                    return ret;
                }
            }
            ...
            for (unsigned i = ni + 1; i < Map::index_size; i++) {
                ...
                if (mi != unused) {
                    ...
                    idx = 0;
                    return lower_bound_at(tree_index(i), &_data._slots[mi], depth, idx);
                }
            }
        ...
        bool visit(Visitor&& v, unsigned depth) const {
            for (unsigned i = 0; i < Map::index_size; i++) {
                uint8_t mi = _data._map[i];
                if (mi != unused) {
                    if (!visit_slot(v, tree_index(i), &_data._slots[mi], depth)) {
                        return false;
                    }
                }
            }
    ...
    struct array_layout {
        ...
        void init(array_layout&& o) noexcept {
            _data.init(std::move(o._data));
        }
        ...
        lower_bound_res lower_bound(index_t& idx, unsigned depth) const noexcept {
            ...
            if (_data.has(ni)) {
                lower_bound_res ret = lower_bound_at(tree_index(ni), &_data._slots[ni], depth, idx);
                if (ret.first != nullptr) {
                    return ret;
                }
            }
            ...
            for (unsigned i = ni + 1; i < Array::size; i++) {
                if (_data.has(i)) {
                    // See comment in table_layout_base about this 0
                    idx = 0;
                    return lower_bound_at(tree_index(i), &_data._slots[i], depth, idx);
                }
            }
        ...
        bool visit(Visitor&& v, unsigned depth) const {
            for (unsigned i = 0; i < Array::size; i++) {
                if (_data.has(i)) {
                    if (!visit_slot(v, tree_index(i), &_data._slots[i], depth)) {
                        return false;
                    }
                }
            }
    ...
    struct table_of_values {
        ...
        void init() noexcept { _present = 0; }
        void init(table_of_values&& o) noexcept {
    ...
    struct array_of_values {
        ...
        void init(array_of_values&& o) noexcept {
            for (unsigned i = 0; i < size; i++) {
                if (o.has(i)) {
                    move_slot(std::move(o._slots[i]), &_slots[i]);
                }
    ...
    struct table_of_node_ptrs {
        ...
        void init(table_of_node_ptrs&& o) noexcept {
            for (unsigned i = 0; i < Size; i++) {
                _idx[i] = o._idx[i];
                move_slot(std::move(o._slots[i]), &_slots[i]);
            }
    ...
    struct array_of_node_ptrs {
        ...
        void init(array_of_node_ptrs&& o) noexcept {
            for (unsigned i = 0; i < size; i++) {
                move_slot(std::move(o._slots[i]), &_slots[i]);
            }
        ...
        bool has(unsigned i) const noexcept { return _slots[i]; }
        void set(unsigned i) noexcept { _size += !_slots[i]; }
    ...
    template <typename Visitor>
    static bool visit_slot(Visitor&& v, index_t idx, const T* val, unsigned depth) {
        return v(idx, *val);
    }

    template <typename Visitor>
    static bool visit_slot(Visitor&& v, index_t idx, const node_head_ptr* ptr, unsigned depth) {
        return (*ptr)->visit(v, depth + 1);
    }

    static lower_bound_res lower_bound_at(index_t index, const T* val, unsigned depth, index_t& idx) noexcept {
        idx = index;
        return lower_bound_res(val, nullptr);
    }

    static lower_bound_res lower_bound_at(index_t index, const node_head_ptr* ptr, unsigned depth, index_t& idx) noexcept {
        return (*ptr)->lower_bound(idx, depth + 1);
    }

    template <typename Fn>
    static bool weed_from_slot(index_t idx, T* val, Fn&& filter, unsigned depth) {
        if (!filter(idx, *val)) {
            return false;
        }
    ...
    template <typename Fn>
    static bool weed_from_slot(index_t idx, node_head_ptr* np, Fn&& filter, unsigned depth) {
        node_head* n = np->raw();
        depth += n->prefix_len();
...
public:
    ...
    void weed(Fn&& filter) {
        if (!_root.is(nil_root)) {
            weed_from_slot(0, &_root, filter, 0);
            if (!_root) {
                _root = &nil_root;
            }
        }
