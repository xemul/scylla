...
future<> client::upload_sink_base::upload_part(memory_data_sink_buffers bufs) {
    ...
    auto permit = co_await _client->claim_memory(bufs.size());
    ...
    req.write_body("bin", size, [this, part_number, bufs = std::move(bufs), p = std::move(permit)] (output_stream<char>&& out_) mutable -> future<> {
        auto permit = std::move(p);
        auto out = std::move(out_);
        std::exception_ptr ex;
