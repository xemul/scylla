...
using allow_dangerous_direct_import_of_cassandra_counters = bool_class<struct allow_dangerous_direct_import_of_cassandra_counters_tag>;
...
class distributed_loader {
    ...
    static future<> populate_keyspace(sharded<replica::database>& db, sharded<db::system_keyspace>& sys_ks, keyspace& ks, sstring ks_name, allow_dangerous_direct_import_of_cassandra_counters);
    static future<std::tuple<table_id, std::vector<std::vector<sstables::shared_sstable>>>>
        get_sstables_from(sharded<replica::database>& db, sstring ks, sstring cf, sstables::sstable_open_config cfg, allow_dangerous_direct_import_of_cassandra_counters,
        noncopyable_function<future<>(global_table_ptr&, sharded<sstables::sstable_directory>&)> start_dir);

public:
    static future<> init_system_keyspace(sharded<db::system_keyspace>&, sharded<locator::effective_replication_map_factory>&, sharded<replica::database>&, allow_dangerous_direct_import_of_cassandra_counters);
    static future<> init_non_system_keyspaces(sharded<replica::database>& db, sharded<service::storage_proxy>& proxy, sharded<db::system_keyspace>& sys_ks, allow_dangerous_direct_import_of_cassandra_counters);
    ...
    static future<std::tuple<table_id, std::vector<std::vector<sstables::shared_sstable>>>>
            get_sstables_from_upload_dir(sharded<replica::database>& db, sstring ks, sstring cf, sstables::sstable_open_config cfg, allow_dangerous_direct_import_of_cassandra_counters);
    static future<std::tuple<table_id, std::vector<std::vector<sstables::shared_sstable>>>>
            get_sstables_from_object_store(sharded<replica::database>& db, sstring ks, sstring cf, std::vector<sstring> sstables, sstring endpoint, sstring type, sstring bucket, sstring prefix, sstables::sstable_open_config cfg, allow_dangerous_direct_import_of_cassandra_counters, std::function<seastar::abort_source*()> = {});
    static future<> process_upload_dir(sharded<replica::database>& db, sharded<db::view::view_builder>& vb, sharded<db::view::view_building_worker>& vbw, sstring ks_name, sstring cf_name, bool skip_cleanup, bool skip_reshape, allow_dangerous_direct_import_of_cassandra_counters);
};
