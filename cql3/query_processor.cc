...
query_processor::execute_schema_statement(const statements::schema_altering_statement& stmt, service::query_state& state, const query_options& options, std::optional<service::group0_guard> guard) {
    ...
    // TODO: either save as a member of statements::schema_altering_statement class (implemented below)
    //       or extend alter_keyspace_statement::prepare_schema_mutations with guard's req id.
    //       Which way to proceed with?
    const_cast<statements::schema_altering_statement&>(stmt).global_req_id = fmt::to_string(request_id);
