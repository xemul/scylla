...
future<> table_helper::setup_table(cql3::query_processor& qp) const {
    ...
    return qp.get_mm().announce_new_column_family(b.build()).discard_result().handle_exception([this] (auto ep) {});;
}
    ...
    if (this_shard_id() == 0) {
        ...
        return seastar::async([&qp, &keyspace_name, replication_factor, &qs, tables] {
            ...
            if (!db.has_keyspace(keyspace_name)) {
                ...
                qp.get_mm().announce_new_keyspace(ksm, api::min_timestamp).get();
            }
