...
public:
    using test_tree = tree<test_key, &test_key::b_hook, test_key_tri_compare, 4, key_search::both, with_debug::yes>;
...
BOOST_AUTO_TEST_CASE(test_semi_empty_tree) {
    test_tree t;

    auto i = t.insert(std::make_unique<test_key>(1), cmp).first;
    i.erase_and_dispose(key_deleter);

    i = t.find(1, cmp);
    BOOST_REQUIRE(i == t.end());
    i = t.lower_bound(1, cmp);
    BOOST_REQUIRE(i == t.end());
    i = t.upper_bound(1, cmp);
    BOOST_REQUIRE(i == t.end());
}
...
BOOST_AUTO_TEST_CASE(test_singular_tree_ptr) {
    ...
    auto it1 = t.insert(std::make_unique<test_key>(1), cmp).first;
    auto it2 = t.insert(std::make_unique<test_key>(2), cmp).first;

    BOOST_REQUIRE(it1.tree_if_singular() == nullptr);
    it2.erase_and_dispose(key_deleter);
    BOOST_REQUIRE(it1.tree_if_singular() == &t);
    it1.erase_and_dispose(key_deleter);
}
...
BOOST_AUTO_TEST_CASE(test_tree_size) {
    ...
    BOOST_REQUIRE(t.calculate_size() == 142);
    t.clear_and_dispose(key_deleter);
}
...
BOOST_AUTO_TEST_CASE(test_swap_between_trees) {
    ...
    test_tree::key_grabber kg(i1);
    auto i1n = t1.insert_before(i2, kg);
