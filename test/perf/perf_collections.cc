...
using test_b_tree = intrusive_b::tree<perf_intrusive_key, &perf_intrusive_key::hook, perf_intrusive_key::tri_compare, 8, intrusive_b::key_search::linear>;
...
public:
    ...
    virtual void drain(int batch) override {
        ...
        auto i = _t.begin();
        while (i != _t.end()) {
            i = i.erase_and_dispose([] (perf_intrusive_key* k) noexcept { delete k; });
            if (++x % batch == 0) {
                seastar::thread::yield();
            }
        }
