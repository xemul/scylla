...
public:
    friend size_t size_for_allocation_strategy(const piggie& o) noexcept { return sizeof(piggie) + o._extra_size; }
    piggie(size_t sz) noexcept : _extra_size(sz) {}
};

static constexpr unsigned nr_seq_allocations = 1024;
static constexpr unsigned nr_iterations = 10000;
...
int main(int argc, char** argv) {
    ...
    return app.run(argc, argv, [&app] {
        return seastar::async([&] {
            ...
            auto& mig = get_standard_migrator<piggie>();
            ...
            for (int iter = 0; iter < nr_iterations; iter++) {
                ...
                void* mem = allocator.alloc(&mig, sizeof(piggie) + sizes[0], alignof(piggie));
                objects[0] = new (mem) piggie(sizes[0]);

                auto start = std::chrono::steady_clock::now();
                for (int i = 1; i < nr_seq_allocations; i++) {
                    void* mem = allocator.alloc(&mig, sizeof(piggie) + sizes[i % nr_sizes], alignof(piggie));
