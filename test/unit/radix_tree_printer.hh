...
class printer {
	...
	static void print_table(const Tbl& table, unsigned depth, std::string id) {
		fmt::print("{:<{}}{}.table{}.{} @{}/{}.{}:", " ", int(depth * 2), id, Tbl::size, table._data.count(), depth, table._data._prefix & 0xffffff00, table._data._prefix & 0xff);
	...
	static void print_map(const Map& map, unsigned depth, std::string id) {
		fmt::print("{:<{}}{}.map @{}/{}.{}:", " ", int(depth * 2), id, depth, map._data._prefix & 0xffffff00, map._data._prefix & 0xff);
	...
	static void print_array(const Arr& array, unsigned depth, std::string id) {
		fmt::print("{:<{}}{}.array.{} @{}/{}.{}:", " ", int(depth * 2), id, array._data.count(), depth, array._data._prefix & 0xffffff00, array._data._prefix & 0xff);
		for (unsigned i = 0; i < tree_t::node_index_limit; i++) {
	...
	static void print(const NT& n, unsigned depth) {
		...
		case node_kind::tiny: return print_table(n._base.template as<typename NT::tiny_node>(), depth, node_id(n._base._head));
		case node_kind::small: return print_table(n._base.template as<typename NT::small_node>(), depth, node_id(n._base._head));
		case node_kind::medium: return print_map(n._base.template as<typename NT::medium_node>(), depth, node_id(n._base._head));
		case node_kind::large: return print_array(n._base.template as<typename NT::large_node>(), depth, node_id(n._base._head));
	...
	static void print(const node_head_t& n, unsigned depth) {
		if (depth == tree_t::leaf_depth) {
			print(n.template as<leaf_node_t>(), depth);
		} else {
			print(n.template as<inner_node_t>(), depth);
		}
	}
