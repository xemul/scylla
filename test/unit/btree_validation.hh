...
template <typename Key, member_hook Key::* Hook, typename Compare, size_t NodeSize>
class validator {
    using key = Key;
    using tree = class tree<Key, Hook, Compare, NodeSize, key_search::both, with_debug::yes>;
    using node = class node<Key, Hook, Compare, NodeSize, key_search::both, with_debug::yes>;

    bool validate(tree& t, node* n, unsigned long& prev_key) {
    ...
    void print_node(node& n, char pfx, int indent) {
        if (n.is_leaf()) {
            fmt::print("{:<{}} {} ({} keys) (l{}{}{}):", pfx, indent, n.id(), n._base.num_keys,
...
public:
    ...
    void print_tree(tree& t, char pfx) {
        fmt::print("/--- TREE ---\n");
        if (t._root != nullptr) {
...
class iterator_checker {
    using tree = class tree<Key, Hook, Compare, NodeSize, key_search::both, with_debug::yes>;
    using validator = class validator<Key, Hook, Compare, NodeSize>;
