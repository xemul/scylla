...
// the ticket about migrating to libunwind: scylladb/seastar#1878
          ...
          shared_token_metadata::mutate_on_all_shards(token_metadata, [host_id] (locator::token_metadata& tm) {
            ...
            sst_loader.start(std::ref(db), std::ref(ss), std::ref(messaging), std::ref(view_builder), std::ref(view_building_worker), std::ref(task_manager), std::ref(sstm), maintenance_scheduling_group,
                sstables_loader::config{.allow_dangerous_direct_import_of_cassandra_counters = cfg->enable_dangerous_direct_import_of_cassandra_counters()}).get();
